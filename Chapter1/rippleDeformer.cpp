...
//camera transformation variables + mouse is pressed
int state = 0, oldX=0, oldY=0, press=0;
...
//mosue click handler
static void  OnMouseDown(GLFWwindow * window, int button, int action,int mods)//(int button, int s, int x, int y)
{
...
void OnRender() {
	...
	tiempo = (float)glfwGetTime () * SPEED;
	//cout<<":tiempo: "<<tiempo;
	...
	shader.Use();
		...
		glUniform1f(shader("time"), tiempo);
